...
jQuery.extend(true,_R, {	
	...
	checkForParallax : function(container,opt) {		
		...
		if (_.type=="3D" || _.type=="3d") {			
			...
			if (opt.sliderType!="carousal" && _.ddd_shadow=="on") {
		...
		function setDDDInContainer(li) {
			if (_.type=="3D" || _.type=="3d") {
				...
				if (opt.sliderType=="carousal") {
					 ...
					 punchgs.TweenLite.set(dddw,{borderRadius:opt.carousal.border_radius});
				}
		...
		if (_.type=="mouse" || _.type=="scroll+mouse" || _.type=="mouse+scroll" || _.type=="3D" || _.type=="3d") {
			...
			container.on('mousemove.hoverdir, mouseleave.hoverdir, trigger3dpath',function(event,data) {				
				...
				if (_.type=="3D" || _.type=="3d") {
					...
					if (opt.sliderType==="carousal") sctor = ".tp-revslider-slidesli .dddwrapper, .tp-revslider-slidesli .dddwrapper-layer, .tp-static-layers .dddwrapper-layer";
					opt.c.find(sctor).each(function() {										
						...
						if (li.hasClass("active-revslide") || opt.sliderType!="carousal")
