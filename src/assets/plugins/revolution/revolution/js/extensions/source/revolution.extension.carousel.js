/********************************************
 * REVOLUTION 5.4.6.5 EXTENSION - Carousal
...
var _R = jQuery.fn.revolution,
	extension = {	alias:"Carousal Min JS",
					name:"revolution.extensions.carousal.min.js",
					min_core: "5.3.0",
					version:"1.2.1"
			  };
...
jQuery.extend(true,_R, {
	...
	// CALCULATE Carousal POSITIONS
	prepareCarousal : function(opt,a,direction,speed) {	

		if (_R.compare_version(extension).check==="stop") return false;

		direction = opt.carousal.lastdirection = dircheck(direction,opt.carousal.lastdirection);		
		
		setCarousalDefaults(opt);	
			
		opt.carousal.slide_offset_target = getActiveCarousalOffset(opt);

		if (speed!==undefined) {
				animateCarousal(opt,direction,false,0);
		} else {
			if (a==undefined) 	
				_R.carousalToEvalPosition(opt,direction);		
			else 	
				animateCarousal(opt,direction,false);	
		}
			
	},

	// MOVE FORWARDS/BACKWARDS DEPENDING ON THE OFFSET TO GET Carousal IN EVAL POSITION AGAIN
	carousalToEvalPosition : function(opt,direction) {
		...
		if (Math.abs(_.slide_offset_target) !==0) 
			animateCarousal(opt,direction,true);
		else {
			_R.organiseCarousal(opt,direction);				
		}	
	},

	// ORGANISE THE Carousal ELEMENTS IN POSITION AND TRANSFORMS
	organiseCarousal : function(opt,direction,setmaind,unli) {	
...
var defineCarousalElements = function(opt) {
	var _ = opt.carousal;
	...
	_.slide_offset = 0; 	
	// SET UL REFERENCE
	_.wrap = opt.c.find('.tp-carousal-wrapper');	
	// COLLECT SLIDES
...
var setCarousalDefaults = function(opt) {	
	...
	if (_.wrap===undefined) defineCarousalElements(opt);	
...
// ANIMATE THE Carousal WITH OFFSETS
var animateCarousal = function(opt,direction,nsae,speed) {
	...
	_.positionanim = punchgs.TweenLite.to(animobj,speed,{from:animobj.to,
		onUpdate:function() {					
			...
			_R.organiseCarousal(opt,direction,false,false);												
		},
		onComplete:function() {	
			...
			_R.organiseCarousal(opt,direction,false,true);	
...
// Carousal INFINITY MODE, DOWN OR UP ANIMATION
...
var getActiveCarousalOffset = function(opt) {
	var ret = 0,
		_ = opt.carousal;
