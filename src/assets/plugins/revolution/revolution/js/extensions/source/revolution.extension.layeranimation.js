		...
		if (t!=undefined && t!="none")
		 if (ap==true || ap=="true" || ap=="on" ||  ap=="1sttime" || an) {	
		 	if (opt.sliderType!=="carousal" || 
		 		(opt.sliderType==="carousal" && opt.carousal.showLayersAllTime==="on" && _nc.closest('li').hasClass("active-revslide")) ||
		 		(opt.sliderType==="carousal" && opt.carousal.showLayersAllTime!=="on" && _nc.closest('li').hasClass("active-revslide"))
		 		) 
	...
	*************************************/	
	animateTheCaptions : function(obj)  {		
		...
		var opt = obj.opt,
			...
			base_offsetx = opt.sliderType==="carousal" ? 0 : opt.width/2 - (opt.gridwidth[opt.curWinRange]*opt.bw)/2,
			base_offsety=0,
		...
		if (index!==undefined && opt.layers[index])
			jQuery.each(opt.layers[index], function(i,a) { 
				...
				if (recall && opt.sliderType==="carousal" && opt.carousal.showLayersAllTime==="on") _R.animcompleted(_t,opt);	    
			});
	...
	********************************************/	
	prepareSingleCaption : function(obj) {
		...
		if (_._ba==="slide") {
			...
			if (_._isstatic && opt.carousal!==undefined && opt.carousal.horizontal_align!==undefined && opt.sliderType==="carousal") {			
				switch (opt.carousal.horizontal_align) {
					case "center":
