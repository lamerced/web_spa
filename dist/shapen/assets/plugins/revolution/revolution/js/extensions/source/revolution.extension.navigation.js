...
jQuery.extend(true,_R, {
	...
	// MANAGE THE NAVIGATION
	 createNavigation : function(container,opt) {
		...
		if (opt.sliderType==="carousal") swipeAction(container,opt,true);
...
var initMouseScroll = function(container,opt) {			
	...
	if (opt.navigation.mouseScrollNavigation!=="on" && opt.navigation.mouseScrollNavigation!=="carousal") return;
	...
	container.on('mousewheel DOMMouseScroll', function(e) {							
			...
			var res = normalizeWheel(e.originalEvent),		
				...
				ret = true;
			if (opt.navigation.mouseScrollNavigation=="carousal") 
				fs = ls = false;								
...
var swipeAction = function(container,opt,vertical) {	
	...
	SwipeOn.swipetp({			
		...
		swipeStatus:function(event,phase,direction,distance,duration,fingerCount,fingerData) {			
			...
			// SWIPE OVER SLIDER, TO SWIPE SLIDES IN Carousal MODE
			if (opt.sliderType==="carousal" && 
				...
				switch (phase) {
					...
					case "move":	
							...
							_R.organiseCarousal(opt,direction,true,true);									
					break;
...
var initArrows = function(container,o,opt) {
	...
	o.padding_top = parseInt((opt.carousal.padding_top||0),0),
	o.padding_bottom = parseInt((opt.carousal.padding_bottom||0),0);
...
var putHinPosition = function(el,o,opt) {
	...
	var elh = el.outerHeight(true),
		elw = el.outerWidth(true),
		bx = o.container=="layergrid" ? opt.sliderType==="carousal" ? 0 : opt.width/2 - (opt.gridwidth[opt.curWinRange]*opt.bw)/2 : 0,
